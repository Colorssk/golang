...
func testQuery() {
	...
	err := DB.Get(&user, sqlstr, 2) // 只需要调用一个方法，就可以吧提取到的数据压入结构体
	if err != nil {
...
func testQueryMulti() {
	...
	err := DB.Select(&user, sqlstr, 1) // 多行查询，存储的结构体设置成切片
	if err != nil {
		fmt.Printf("get failed, err:%v\n", err)
		return
	}
...
func testSqlInject() {
	//queryDB("abc' or 1 = 1 #")// 可以脱库
	//queryDB("name=abc' and (select count(*) from user ) < 10#")// 猜数据库的大小
	//queryDB("name=123' union select *from user #")// 并集，脱库
